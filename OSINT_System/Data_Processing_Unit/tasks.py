...
from Data_Processing_Unit.models import News_Fetched_Data,\
    ...
    Linkedin_Company,\
    Instagram_Posts,\
    Instagram_Person,Facebook_Person
    ...
    try:
        for news in news_site:
            ...
            if(response is not None):
                insert_news_data_to_mongodb(response,news,top,GTR_ID=0)
...
def fetch_instagram_person(GTR,author_type,author_account):
    try:
        ...
        if(response is not None):
            print(response)
            insert_instagram_profile_to_mongodb(GTR,response,author_account)
...
def fetch_facebook_person(GTR,pattern_type,author_account,):
    ...
    try:
        ...
        insert_facebook_person_mongodb(GTR,responses,author_account)
...
def fetch_twitter_tweets(GTR,tweet_type='phrase',username='itsaadee',hashtag=['pmln','pti'],phrase='living my dreams',date='2015-1-1',location='NYC',distance='15mi'):
    ...
    if (response is not None):
        if(tweet_type != 'refferencing' or tweet_type != 'to' or tweet_type != 'from'):
            insert_search_tweets_mongodb(GTR,hashtag,phrase,date,distance,response,tweet_type)
...
def fetch_twitter_person_tweets(GTR,tweet_type='to',id='23245',name='atifaslam',account='itsaadee',url = 'https://twitter.com/itsaadee'):
    ...
    if (response is not None):
        if(tweet_type != 'refferencing' or tweet_type != 'to' or tweet_type != 'from'):
            insert_person_tweets_mongodb(GTR,id,name,account,response,response_profile,tweet_type,url)
        else:
            pass
    else:
        print('response is empty or task did not executed successfully ')
...
def fetch_linkedin_person(GTR,author_account):
    try:
        ...
        if(response is not None):
            #print(response)
            insert_linkedin_person_to_mongodb(GTR,response,author_account)
...
def fetch_linkedin_company(GTR,author_account):
    try:
        ...
        if(response is not None):
            print(response)
            insert_linkedin_company_to_mongodb(GTR,response,author_account)
...
@huey.signal(signals.SIGNAL_ERROR, signals.SIGNAL_LOCKED, signals.SIGNAL_CANCELED, signals.SIGNAL_REVOKED)
...
def insert_news_data_to_mongodb(response,news_site,top,GTR_ID):
...
def insert_facebook_person_mongodb(GTR,responses,author_account):
    ...
    fp.target_update_count = facebook_author_identification(author_account)
    ...
    if(res_overview is not None and res_posts is not None and res_checkIns is not None and res_closeAssociates is not None and res_interests is not None):
        fp.profile_summary = get_summary_of_facebook_profile(res_overview,res_posts,res_checkIns,res_closeAssociates,res_interests)
    ...
    if(res_posts is not None):
        fp.post_frequency_graph = get_graph_of_facebook_profile(res_posts)
...
def insert_instagram_profile_to_mongodb(GTR_ID,response,author_account):
    ...
    ip.target_update_count = instagram_author_identification(author_account)
    ...
    ip.profile_summary = get_instagram_summary(response)
...
def insert_search_tweets_mongodb(GTR,hashtags,phrase,date,distance,response,tweet_type):
...
def insert_person_tweets_mongodb(GTR,idn,name,account,response,response_profile_a,tweet_type,url):

    tweet_objects_list = []

    ts = Twitter_Person()

    ts.profile_summary = get_twitter_profile_summary(response_profile_a)

    ts.frequent_hashtags = get_frequent_hashtags_in_tweets(response_profile_a)
    ts.interests = get_author_interests_from_tweets(response_profile_a)
    ...
    ts.target_update_count = twitter_author_identification(account)

    close_associates,tweets_frequency = get_graph_of_tweets(response,account)
...
def insert_linkedin_company_to_mongodb(GTR,response,author_account):
    ...
    if(response is not None):
        ...
        lc.target_update_count = linkedin_company_identification(author_account)
...
def insert_linkedin_person_to_mongodb(GTR,response,author_account):
    ...
    if(response is not None):
        ...
        lp.profile_summary = get_linkedin_person_profile_summary(response)
        ...
        lp.target_update_count = linkedin_person_identification(author_account)
...
def insert_twitter_trends_to_mongodb(response):
    pass




def facebook_author_identification(author_account):
...
def instagram_author_identification(author_account):
    #this function takes the mongodb table and fetch all of its
    #objects and try to find the match for the given author_name
...
def twitter_author_identification(author_account):
    #this function takes the mongodb table and fetch all of its
    #objects and try to find the match for the given author_name
...
def linkedin_person_identification(author_account):
    #this function takes the mongodb table and fetch all of its
    #objects and try to find the match for the given author_name
...
def linkedin_company_identification(author_account):
    #this function takes the mongodb table and fetch all of its
    #objects and try to find the match for the given author_name
...
def get_summary_of_facebook_profile(dataoverview, facebook_pofile_posts, facebook_profile_checkins,facebook_profile_close_associates, facebook_profile_interests):
    ...
    yaxis = [janpost, febpost, marchpost, aprilpost, maypost, junepost, julypost, augpost, seppost, octpost, novpost,
             decpost]
...
def get_graph_of_facebook_profile(facebook_posts_data):
    ...
    noOfPost= [janpost, febpost, marchpost, aprilpost, maypost, junepost, julypost, augpost, seppost, octpost, novpost, decpost]
    #print(noOfPost)



    yaxis = [janpost, febpost, marchpost, aprilpost, maypost, junepost, julypost, augpost, seppost, octpost, novpost, decpost]
...
def get_twitter_profile_summary(twitter_profile_data):
...
def get_graph_of_tweets(twitter_tweets_data, username_of_targeted_account):
...
def get_frequent_hashtags_in_tweets(twitter_tweets_data):
    tweets_dictionary = twitter_tweets_data['tweets']
...
def get_author_interests_from_tweets(twitter_tweets_data):
    ...
    dict4 = {**lawpercentage, **religionpercentage, **healthpercentage, **WeatherDisasterEmergencypercentage,
             **agenciespercentage, **domesticsecuritypercentage, **borderviolencepercentage, **cybersecuritypercentage,
             **infrastructuresecuritypercentage, **nuclearpercentage, **terrorismpercentage, **politicspercentage,
             **sportspercentage, **technologypercentage, **artspercentage, **businesspercentage, **educationpercentage,
...
def get_sentiment_analysis_of_tweets(custom_tweet):
...
#_______________________________________________________Linkedin Person Profile Analysis_________________________________

def get_linkedin_person_profile_summary(linkenin_data):
...
#_______________________________________________________Instagram Person Profile Analysis_________________________________

def get_instagram_summary(instagram_data):
